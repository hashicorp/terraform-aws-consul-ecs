...
module "dc1" {
  ...
  region          = var.region
  vpc             = module.dc1_vpc
  // TODO primary_datacenter = local.primary_datacenter
  ...
  additional_dns_names = ["${var.name}-${var.datacenter_names[1]}-consul-server.consul-${var.datacenter_names[1]}"]
...
module "dc2" {
  ...
  vpc             = module.dc2_vpc
  // TODO primary_datacenter = local.primary_datacenter
  // TODO primary_gateways   = ["${module.dc1_gateway.lb_dns_name}:8443"]
  ca_cert_arn    = aws_secretsmanager_secret.ca_cert.arn
  ca_key_arn     = aws_secretsmanager_secret.ca_key.arn
  gossip_key_arn = aws_secretsmanager_secret.gossip_key.arn

  additional_dns_names = ["${var.name}-${var.datacenter_names[0]}-consul-server.consul-${var.datacenter_names[0]}"]
