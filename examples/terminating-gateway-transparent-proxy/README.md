# Terminating Gateways on ECS (Transparent Proxy)

This example demonstrate accessing non mesh destinations services from mesh tasks with the help of terminating gateways setup with transparent proxy deployed as ECS tasks in EC2 launch type.
...
[Application UI](https://github.com/hashicorp/terraform-aws-consul-ecs/blob/main/_docs/terminating-gateway-client-tproxy-ui.png)
...
If you notice the `UPSTREAM_URIS` environment variable in the client app's container definition, it is defined according to Consul's DNS nomenclature. The DNS resolution in the client app's container takes place with Consul and Consul returns back the virtual IP of the upstream external server app. The request is then transparently routed to the client app's proxy which is then redirected to the terminating gateway instance and then to the external server app.
