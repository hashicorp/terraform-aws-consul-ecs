...
locals {
  example_server_app_name = "${var.name}-example-server-app"
...
module "example_server_app" {
  ...
  container_definitions = [{
    ...
    image            = "ghcr.io/lkysow/fake-service:v0.21.0" // Using a clone to avoid rate limits
    essential        = true
...
resource "aws_ecs_service" "example_server_app" {
  ...
  desired_count   = var.server_instances_per_group
